...
**Valor:** Capacitar os entregadores com um aplicativo dedicado para gerenciar suas entregas de forma eficiente, desde a aceitação até a finalização, e fornecer rastreamento em tempo real para clientes e produtores, melhorando a transparência e a experiência de todos os usuários.

### Funcionalidades e Distribuição

#### 1. Autenticação e Perfil do Entregador (App Entregador)
- **Backend:**
  - Endpoint de login/logout específico para entregadores.
  - Endpoint para visualizar/editar perfil básico do entregador (nome, contato, dados do veículo se aplicável).
- **Frontend (App Entregador):**
  - Telas de login e logout.
  - Tela de visualização e edição de perfil.
- **Responsável:** [@andrebarceloschagas](https://github.com/andrebarceloschagas) (Frontend + Backend)

---

#### 2. Listagem, Aceitação e Detalhes de Pedidos (App Entregador)
- **Backend:**
  - Endpoint para listar pedidos disponíveis para entrega (filtrados por status "Aguardando Coleta", proximidade, etc.).
  - Endpoint para o entregador aceitar um pedido (mudando seu status e atribuindo ao entregador).
  - Endpoint para o entregador recusar/liberar um pedido (se aplicável).
  - Endpoint para buscar detalhes completos de um pedido atribuído (endereço do produtor para coleta, endereço do cliente para entrega, itens, observações, valor a receber se houver).
- **Frontend (App Entregador):**
  - Tela de listagem de pedidos disponíveis com informações chave (distância, valor da entrega, resumo).
  - Botões para aceitar/recusar pedidos.
  - Tela de detalhes do pedido com todas as informações necessárias para a coleta e entrega.
- **Responsável:** [@raphaelsales](https://github.com/raphaelsales) (Frontend + Backend)

---

#### 3. Atualização de Status e Rastreamento da Entrega (App Entregador e Plataforma Web)
- **Backend:**
  - Endpoints para o entregador atualizar o status do pedido (Ex: "Aguardando Coleta" -> "Coletado", "Coletado" -> "A Caminho", "A Caminho" -> "Entregue", "A Caminho" -> "Problema na Entrega").
  - Lógica para registrar o horário de cada mudança de status.
  - Gatilhos para notificar cliente e produtor sobre mudanças de status relevantes (via sistema de notificações da plataforma).
- **Frontend (App Entregador):**
  - Interface clara para o entregador atualizar o status da entrega com um toque.
  - Exibição do endereço de coleta/entrega com opção de abrir em app de mapas externo (Google Maps, Waze).
- **Frontend (Cliente/Produtor - Plataforma Web):**
  - Atualização em tempo real (ou com refresh) da visualização do status do pedido nas telas de "Histórico de Pedidos" e "Detalhes do Pedido" para refletir as atualizações feitas pelo entregador.
- **Responsável:** [@thiagogonzagadev](https://github.com/thiagogonzagadev) (Backend e Frontend App Entregador), [@uGonzaguinha](https://github.com/uGonzaguinha) (Frontend Plataforma Web para visualização do rastreamento)

---

#### 4. Histórico de Entregas e Ganhos (App Entregador)
- **Backend:**
  - Endpoint para listar o histórico de entregas realizadas pelo entregador (pedidos concluídos, cancelados com justificativa, etc.).
  - Endpoint para calcular e exibir os ganhos do entregador por período ou por entrega (se a lógica de pagamento ao entregador for implementada).
- **Frontend (App Entregador):**
  - Tela de histórico de entregas com detalhes de cada uma.
  - Seção para visualização de ganhos (resumo diário, semanal, mensal).
- **Responsável:** [@HeloCris](https://github.com/HeloCris) (Frontend + Backend)

---

#### 5. Notificações Push para Entregadores (App Entregador)
- **Backend:**
  - Integração com serviço de notificação push (ex: Firebase Cloud Messaging).
  - Endpoints/Lógica para disparar notificações para entregadores sobre:
    - Novos pedidos disponíveis na sua área.
    - Pedido atribuído.
    - Lembretes ou atualizações importantes.
- **Frontend (App Entregador):**
  - Configuração para receber notificações push.
  - Exibição das notificações dentro do app ou na barra de status do dispositivo.
- **Responsável:** [@andrebarceloschagas](https://github.com/andrebarceloschagas) (Frontend + Backend - como complemento à infraestrutura do entregador)
...
| Membro               | Frontend                                                                                      | Backend                                                                                             |
|----------------------|-----------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------|
| @andrebarceloschagas | App Entregador: Autenticação, Perfil, Notificações Push                                       | App Entregador: Autenticação, Perfil, Notificações Push, Integração com serviço de Notificação Push |
| @raphaelsales        | App Entregador: Listagem de Pedidos, Aceitação/Recusa, Detalhes do Pedido                     | App Entregador: Endpoints para Listagem, Aceitação/Recusa, Detalhes de Pedidos                      |
| @thiagogonzagadev    | App Entregador: Interface de Atualização de Status, Exibição de Rota/Link para Mapas          | Endpoints de Atualização de Status, Lógica de Rastreamento, Gatilhos de Notificação Cliente/Produtor |
| @HeloCris            | App Entregador: Tela de Histórico de Entregas, Tela de Ganhos                                 | Endpoints para Histórico de Entregas e Cálculo/Exibição de Ganhos                                   |
| @uGonzaguinha        | Plataforma Web (Cliente/Produtor): Atualização da Visualização do Status/Rastreamento do Pedido | (Foco no Frontend Web para esta tarefa específica de visualização)                                  |

---

### Resumo das Entregas Sprint 4

- Aplicativo do entregador funcional com login e gerenciamento básico de perfil.
- Entregadores podem visualizar pedidos disponíveis e aceitá-los através do app.
- Entregadores podem atualizar o status da entrega (coletado, a caminho, entregue, problema) via app.
- Clientes e produtores podem visualizar o rastreamento atualizado do pedido em suas respectivas interfaces na plataforma web.
- Entregadores têm acesso ao seu histórico de entregas e um resumo de ganhos (se aplicável no MVP) no app.
- Sistema de notificações push implementado para alertar entregadores sobre novos pedidos e atualizações.
